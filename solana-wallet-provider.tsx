...
export function SolanaWalletProvider({ children }: { children: React.ReactNode }) {
  ...
  function refreshWallets() {
    ...
    if (active) {
      ...
      setConnectedWalletId(active.id);
    }
  }
    ...
    try {
      if (target.provider.disconnect) {
        try {
          ...
          // ignore pre-connect disconnect failures
        }
      }
      ...
      setConnectedWalletId(target.id);
    } catch (connectError) {
      const message =
  ...
  async function disconnectWallet() {
    const likelyConnectedWallets = wallets.filter(
      (wallet) =>
        wallet.id === connectedWalletId ||
        wallet.name === connectedWalletName ||
        wallet.provider.publicKey?.toString() === publicKey,
    );
    const targets = likelyConnectedWallets.length > 0 ? likelyConnectedWallets : wallets;
  ...
  useEffect(() => {
    ...
    function onAccountChanged(nextKey: unknown) {
      ...
      if (
        ...
        setConnectedWalletId((previous) => previous ?? "detected-solana");
      } else {
        setPublicKey(null);
        setConnectedWalletName(null);
        setConnectedWalletId(null);
